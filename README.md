...
A reproducible build will produce the same binary result every time a given set of source is built. To acheive reproducibility 
it is necessary to have source that follows reproduciblble rules, like no embedded build paths or date-time stamps, 
and a toolchain that also produces consistent results, for example some linkers embed RPATH data into binariesm while, 
...
Once you have a reproducible build, you can effienctly track changes in binaries by taking a sha256sum of each binary
and comparing against known results.  Even if you have no knonw results, you can use differing versions of the toolchain 
...
An audiable build enviornment onains nothing that cannot be explicitly accounted for.
...
but it is possible to restore audiability by using the base package system (ie dpkg) to install packages individually. 
...
become obsolete.  The distro packages are not an active part of the trusted toolchain.
...
```
sudo docker save candiate -o /tmp/secure-build-bootstrap.tar
